	...
	<div style="height: 100%;background: #F2F6FC;">
		<el-container style="height: 100%;">
			<el-header class="header">
				...
				<el-button-group style="position: absolute;right: 20px;top: 15px;">
			...
			<el-main style="height: 100%;box-sizing: border-box;">
				<el-card class="box-card login" style="width: 1000px;margin: 0 auto;">
					<div style="line-height: 30px;font-size: 14px;">
						{{audio.filename}}
						(<span style="color: red;">{{audio.filesize}}</span>)
					</div>
					<audio :src="url" controls="controls" style="width: 100%;">您的浏览器不支持音频播放</audio>
				</el-card>
    		</el-main>
		</el-container>
		<!--弹出框-->
...
<script>
	...
	export default {
        ...
        created(){
        	this.audio.id=this.$route.query.id;
        	this.audio.filename=this.$route.query.filename;
        	this.audio.filesize=this.$route.query.filesize;
        	this.token=this.$route.query.token;
        	
        	this.url=this.baseurl+"disk/fileopen/writeByte?fileid="+this.audio.id+"&token="+this.token;
        },
        methods:{
        	download(){
				location.href=this.baseurl+"disk/filedownload/download?fileid="+this.audio.id+"&token="+this.token;
			},
			share(){
				var name=this.audio.filename;
